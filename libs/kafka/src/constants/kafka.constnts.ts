const kafkaBrokersEnv =
  process.env.KAFKA_BROKERS ?? process.env.KAFKA_BROKER ?? 'localhost:9093';

export const KAFKA_BROKERS = kafkaBrokersEnv
  .split(',')
  .map((broker) => broker.trim())
  .filter(Boolean);

export const KAFKA_BROKER = KAFKA_BROKERS[0];
